...
from sam3.entrypoint import ProcessingResult, Sam3Entrypoint
...
class Sam3Wrapper:
    ...
    def __init__(
        ...
        self.entrypoint = Sam3Entrypoint(
            default_output_dir=tmp_base, min_video_frames=min_frames, min_chunk_overlap=chunk_overlap
        )
        ...
        if verbose:
            logger.info("Sam3Wrapper initialized (delegating to Sam3Entrypoint)")
    ...
    def prepare_video(self, video_path: str) -> dict[str, Any]:
        ...
        feasibility = self.entrypoint._check_feasibility(memory_needed, video_meta.total_frames, is_video=True)
        ...
        if feasibility.requires_chunking:
            chunks_new = self.entrypoint._generate_chunks(video_meta_new, feasibility.chunk_size, self.chunk_overlap)
            chunks = [
                ChunkInfo(
                    chunk_id=i,
    ...
    def cleanup_video_workspace(self, video_path: str | None = None):
    ...
    def process_image(
        ...
        return self.entrypoint.process_image(
            image_path=image_path, prompts=prompts, boxes=boxes, output_dir=output_dir or self.tmp_base
        )
