...
class Sam3Entry:
    def __init__(self, bpe_path: str | None = None, num_workers: int | None = None):
    ...
    def _save_prompt_video_metadata(self, metadata: dict, video_name: str, prompt: str, chunk_idx: int | None = None):
        file_name = f"{prompt}.json"

        meta_dir = (
            Path(TEMP_DIR) / video_name / "metadata" / str(chunk_idx)
            if chunk_idx is not None
            else Path(TEMP_DIR) / video_name / "metadata"
        )
        meta_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _update_prompt_video_metadata(self, new_data: dict, video_name: str, prompt: str, chunk_idx: int | None = None):
        file_name = f"{prompt}.json"
        ...
        if not meta_path.is_file():
            ...
            logger.warning("Saving new metadata instead of updating.")
    ...
    def _save_prompt_video_data(self, result_prompt, object_ids, frame_objects, prompt, video_name, video_meta):
        prompt_masks_dir = self._save_prompt_video_mask_data(result_prompt, object_ids, prompt, video_name, video_meta)
        metadata = {"mask_dir": str(prompt_masks_dir), "object_ids": list(object_ids), "frame_objects": frame_objects}
    ...
    def _process_video_prompt(
        self, driver, session_id, propagation_direction, prompt, video_name, video_meta: dict[str, Any]
    ):
        ...
        return self._save_prompt_video_data(result_prompt, object_ids, frame_objects, prompt, video_name, video_meta)
    ...
    def _process_video_chunk(
        ...
        for prompt in prompts:
            ...
            prompt_meta_path = self._process_video_prompt(
                driver, session_id, propagation_direction, prompt, video_name, video_meta
            )
    ...
    def _process_video_in_chunks(
        ...
        for chunk in video_chunks:
            ...
            logger.info(
                f"Extracting chunk {chunk['chunk']} (frames {chunk['start']} to {chunk['end']}) to {chunk_path}"
            )

            _ = ffmpeg_lib.create_video_chunk(
                input_video=video_path, output_video=chunk_path, start_frame=chunk["start"], end_frame=chunk["end"]
            )
    ...
    def process_video_with_prompts(
        ...
        if device not in ["cpu", "cuda"]:
            raise ValueError(f"Unsupported device type: {device}. Supported types are 'cpu' and 'cuda'.")

        logger.info(f"Processing video: {video_path} with prompt: {prompts}")

        video_meta, video_chunks = memory_manager.chunk_plan_video(video_path, device=device, chunk_spread=chunk_spread)
        ...
        if len(video_chunks) == 0:
            ...
            logger.info("Processing video in a single chunk.")
