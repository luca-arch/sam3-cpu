...
class VideoPostProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"PostProcessor initialized: {len(chunk_results)} chunks, "
    ...
    def process(self, prompts: list[str]):
        ...
        logger.debug("    Saving combined mapping metadata...")
    ...
    def _save_combined_mapping_metadata(self, all_mappings: dict[str, dict], all_iou_matrices: dict[str, dict] = None):
        ...
        metadata = {
            ...
            "mappings": {"by_chunk": chunk_based, "by_prompt": prompt_based},
    ...
    def _match_chunks(self, chunk_i: dict, chunk_j: dict, prompt: str) -> dict[int, int]:
        """
        ...
        if not object_ids_i or not object_ids_j:
            logger.info(
                f"          No objects to match (chunk_{chunk_i_id}: {len(object_ids_i)}, chunk_{chunk_j_id}: {len(object_ids_j)})"
            )
            return {}

        logger.info(
            f"          Matching chunk_{chunk_i_id} ({len(object_ids_i)} objs) -> chunk_{chunk_j_id} ({len(object_ids_j)} objs) with {self.overlap_frames} overlap frame(s)"
        )
        ...
        if injected_ids_j:
            ...
            for obj_id in injected_ids_j:
                if obj_id in set_i and obj_id in object_ids_j:
                    ...
                    logger.info(
                        f"            ✅ Injected match: chunk_{chunk_i_id} obj_{obj_id} -> chunk_{chunk_j_id} obj_{obj_id} (deterministic)"
                    )
        ...
        if remaining_i and remaining_j:
            logger.info(
                f"          Phase 2: IoU matching for {len(remaining_i)} remaining chunk_i objs vs {len(remaining_j)} chunk_j objs"
            )
            ...
            logger.info(
                f"          Loaded overlap masks: chunk_{chunk_i_id}={len(overlap_masks_i)} frames, chunk_{chunk_j_id}={len(overlap_masks_j)} frames"
            )
            ...
            if overlap_masks_i and overlap_masks_j:
                ...
                for id_i in remaining_i:
                    ...
                    for id_j in remaining_j:
                        ...
                        logger.debug(
                            f"              IoU(obj_{id_i}, obj_{id_j}) = {best_iou_candidate:.4f} (max over {len(ious)} frames)"
                        )
                    ...
                    if best_iou > self.iou_threshold:
                        ...
                        logger.info(
                            f"            ✅ IoU match: chunk_{chunk_i_id} obj_{id_i} -> chunk_{chunk_j_id} obj_{best_id_j} (IoU={best_iou:.3f})"
                        )
                    else:
                        logger.debug(
                            f"            No match for obj_{id_i} (best IoU={best_iou:.4f} < threshold={self.iou_threshold})"
                        )

        return mappings

    def _load_overlap_frames_from_pngs(
        self, chunk: dict, prompt: str, position: str = "end"
                ...
                try:
                    ...
                    if mask_image is not None and mask_image.size > 0:
                        ...
                        if frame_idx == 0:  # Log only first frame for brevity
                            ...
                            logger.debug(
                                f"            ✓ chunk_{chunk_id}/object_{obj_id}: {len(selected_pngs)} frames, first has {nonzero} pixels"
                            )
                except Exception as e:
                    logger.warning(f"            Failed to read mask {png_path}: {e}")

        logger.info(
            f"          Loaded {len(overlap_frames)} overlap frame(s) from chunk_{chunk_id} ({len(object_ids)} objects)"
        )
    ...
    def _match_frame_masks(self, masks_i: dict[int, np.ndarray], masks_j: dict[int, np.ndarray]) -> dict[int, int]:
        """
    ...
    def _load_chunk_overlap_masks(self, chunk: dict, prompt: str, position: str = "end") -> list[dict[int, np.ndarray]]:
        """
        ...
        logger.info(
            f"          Loading {position} overlap masks for chunk {chunk_id}, "
    ...
    def _stitch_masks_for_prompt(self, prompt: str, prompt_mappings: dict):
        ...
        logger.info(
            f"      Built {len(chains)} chains for '{prompt}': {tracked_chains} tracked across chunks, {singleton_chains} unmatched singletons"
        )
            ...
            try:
                ...
                for chain_idx, (chunk_id, local_id) in enumerate(chain):
                    ...
                    frames_copied = self._copy_frames_from_mask_pngs(
                        mask_object_dir, writer, skip_first_n=(self.overlap_frames if chain_idx > 0 else 0)
                    )
    ...
    def _build_chains_from_mappings(self, prompt: str, prompt_mappings: dict) -> dict:
        ...
        for start_obj_id in first_chunk_objects:
            ...
            if len(chains[start_obj_id]) > 1:
                logger.info(
                    f"          Tracked chain object_{start_obj_id}: {len(chains[start_obj_id])} chunks - {chains[start_obj_id]}"
                )
        ...
        for chunk_id, local_id in all_objects:
            if (chunk_id, local_id) not in processed:
                ...
                logger.debug(
                    f"          Unmatched object: chunk_{chunk_id} object_{local_id} -> output object_{unmatched_counter}"
                )
                unmatched_counter += 1

        return chains

    def _get_chunk_result(self, chunk_id: int) -> dict | None:
    ...
    def _copy_frames_from_mask_pngs(self, mask_dir: Path, writer: cv2.VideoWriter, skip_first_n: int = 0) -> int:
        """
            ...
            try:
                ...
                if mask_image is not None and mask_image.size > 0:
                    ...
                    if len(mask_image.shape) == 2 and mask_image.dtype == np.uint8:
                        ...
                        logger.warning(
                            f"          Invalid mask shape/dtype: {png_file} - shape={mask_image.shape}, dtype={mask_image.dtype}"
                        )
                else:
