...
from sam3.__globals import BPE_PATH, DEFAULT_NUM_WORKERS, DEFAULT_PROPAGATION_DIRECTION
...
class VideoProcessor:
    ...
    def process_with_prompts(
        ...
        if len(video_chunks) == 1:
            ...
            chunk_results = self._process_single_chunk_video(
                prompts=prompts, video_metadata=video_metadata, propagation_direction=propagation_direction
            )
        else:
        ...
        per_chunk_timing = [cr.get("timing", {}) for cr in chunk_results if "timing" in cr]
    ...
    def _create_chunk_plan(self, chunk_spread: str) -> tuple:
        ...
        video_metadata, video_chunks = memory_manager.chunk_plan_video(
            video_file=str(self.video_path), device=self.device, chunk_spread=chunk_spread
        )

        logger.debug(
            f"  Video: {video_metadata['width']}x{video_metadata['height']}, "
    ...
    def _process_single_chunk_video(
        ...
        {"chunk": 0, "start": 0, "end": video_metadata["nb_frames"] - 1}
        ...
        chunk_result = chunk_processor.process_with_prompts(
            prompts=prompts, propagation_direction=propagation_direction
        )
    ...
    def _process_multiple_chunks(
        ...
        for chunk_info in video_chunks:
            ...
            logger.info(f"  Processing chunk {chunk_id + 1}/{len(video_chunks)}: frames {start_frame}-{end_frame}")
            ...
            chunk_result = chunk_processor.process_with_prompts(
                prompts=prompts, propagation_direction=propagation_direction, prev_chunk_data=prev_chunk_data
            )
            ...
            if prev_chunk_data:
                ...
                logger.debug("    No carry-forward data for next chunk")
    ...
    def _visualize_results(self, chunk_results: list[dict[str, Any]], prompts: list[str]):
        """
    ...
    def _move_temp_to_output(self):
        ...
        if self.video_temp_dir.exists():
            shutil.copytree(self.video_temp_dir, temp_preserved_dir, dirs_exist_ok=True)
