...
class ImageProcessor:
    ...
    def __init__(self, output_dir: Path, device: str, bpe_path: str = BPE_PATH, num_workers: int = DEFAULT_NUM_WORKERS):
    ...
    def driver(self) -> Sam3ImageDriver:
        ...
        if self._driver is None:
            ...
            self._driver = Sam3ImageDriver(bpe_path=self.bpe_path, num_workers=self.num_workers)
    ...
    def process_with_prompts(self, image_paths: list[Path], prompts: list[str]) -> dict[str, Any]:
        ...
        for idx, image_path in enumerate(image_paths):
            ...
            image_result = self._process_single_image_with_prompts(image_path=image_path, prompts=prompts)
    ...
    def _process_single_prompt(
        ...
        inference_state = self.driver.prompt_and_predict(
            processor=processor, inference_state=inference_state, prompt=prompt
        )
        ...
        if num_objects > 0:
            self._save_masks(
                masks=masks, scores=scores, boxes=boxes, object_ids=object_ids, prompt_masks_dir=prompt_masks_dir
            )
    ...
    def process_with_boxes(self, image_path: Path, boxes: list[list[float]], box_labels: list[int]) -> dict[str, Any]:
        ...
        inference_state = self.driver.prompt_multi_box_with_labels(
            image=image, processor=processor, inference_state=inference_state, boxes_xywh=boxes, box_labels=box_labels
        )
        ...
        if num_objects > 0:
            ...
            self._save_masks(
                masks=masks, scores=scores, boxes=result_boxes, object_ids=object_ids, prompt_masks_dir=boxes_masks_dir
            )
