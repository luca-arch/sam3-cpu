...
class Sam3Entrypoint:
    ...
    def __init__(
        ...
        # Lazy-loaded drivers
        self._image_driver: Sam3ImageDriver | None = None
        self._video_driver: Sam3VideoDriver | None = None
    ...
    @staticmethod
    def _parse_time_or_frame(value: str | int | float, fps: float | None = None) -> dict:
    ...
    def _calculate_memory_needed(self, width: int, height: int, num_frames: int = 1, is_video: bool = False) -> float:
        ...
        if self.verbose:
            logger.debug("Memory calculation:")
    ...
    def _check_feasibility(
        self, memory_needed_gb: float, total_frames: int = 1, is_video: bool = False
    ) -> FeasibilityResult:
        """
        ...
        if enough_available:
            ...
            if self.verbose:
                logger.info(
                    f"✓ Memory check passed: {required_memory_gb:.2f} GB needed, "
            ...
            if self.verbose:
                logger.warning("⚠ Insufficient available memory")
            ...
            if is_video:
                ...
                if self.verbose:
                    logger.info("📊 Chunking will be required (detailed info shown during processing)")
            else:
    ...
    def _get_video_metadata(self, video_path: str) -> VideoMetadata:
        ...
        try:
            ...
            raise RuntimeError(f"Invalid video metadata values: {e}") from e
    ...
    def _generate_chunks(self, video_meta: VideoMetadata, chunk_size: int, overlap: int) -> list[ChunkInfo]:
        """
        ...
        while start_frame < video_meta.total_frames:
            ...
            if self.verbose:
                logger.debug(
                    f"Chunk {chunk_id}: frames {start_frame}-{end_frame} ({num_frames} frames, {duration_sec:.2f}s)"
                )
    ...
    def _extract_video_chunk(self, video_path: str, chunk_info: ChunkInfo, output_path: str) -> str:
        ...
        try:
            ...
            cmd = [
                ...
                "-ss",
                str(chunk_info.start_time_sec),  # Start time (before -i for speed)
                "-i",
                video_path,
                "-t",
                str(chunk_info.duration_sec),  # Duration
                "-c:v",
                "libx264",  # H.264 codec
                "-qp",
                "0",  # Lossless mode (QP=0)
                "-preset",
                "veryfast",  # Speed up encoding
                "-c:a",
                "copy",  # Copy audio (if any)
                output_path,
            ]
    ...
    ) -> tuple[list[str], list[str] | None, list[list[float]] | None]:
    ...
    def _validate_video_input(
        ...
        if points is not None:
            ...
            if len(points) != len(point_labels):
                raise ValueError(f"Number of points ({len(points)}) must match number of labels ({len(point_labels)})")
        ...
        if segments is not None:
            ...
            for i, segment in enumerate(segments["segments"]):
                ...
                if not (has_frames or has_time):
                    raise ValueError(
                        f"Segment {i} must have either (start_frame, end_frame) or (start_time_sec, end_time_sec)"
                    )
                ...
                if not (has_prompts or has_points):
                    raise ValueError(f"Segment {i} must have either 'prompts' or ('points' and 'labels')")

                # Validate points/labels consistency
                if has_points:
                    if len(segment["points"]) != len(segment["labels"]):
                        raise ValueError(f"Segment {i}: number of points must match number of labels")
    ...
    def process_image(
        ...
        image_path: str | list[str],
        prompts: str | list[str] | None = None,
        boxes: list[float] | list[list[float]] | None = None,
        ...
        for img_path in image_paths:
            try:
                ...
                if prompts is not None:
                    ...
                    for prompt, state in results.items():
                        ...
                        if "masks" in state and len(state["masks"]) > 0:
                            ...
                            saved_files = self._save_image_masks(masks, obj_ids, img_output_dir, prompt)
                ...
                # Process with boxes (scenarios b, d)
                    ...
                    if len(boxes) == 1:
                        # Single box
                        state = driver.prompt_bounding_box(img, processor, inference_state, boxes[0])
                    else:
                        # Multiple boxes
                        if box_labels is None:
                            box_labels = [1] * len(boxes)  # Default all positive

                        state = driver.prompt_multi_box_with_labels(img, processor, inference_state, boxes, box_labels)
                    ...
                    if "masks" in state and len(state["masks"]) > 0:
                        ...
                        saved_files = self._save_image_masks(masks, obj_ids, img_output_dir, "box")
                        all_mask_files.extend(saved_files)
                        all_object_ids.extend(obj_ids)
    ...
    def process_video_with_prompts(
        ...
        frame_from: int | str | float | None = None,
        frame_to: int | str | float | None = None,
    ) -> ProcessingResult:
        ...
        if frame_from is not None or frame_to is not None:
            ...
            if actual_frame_from < 0 or actual_frame_to >= video_meta.total_frames:
                raise ValueError(
                    f"Frame range {actual_frame_from}-{actual_frame_to} out of bounds (0-{video_meta.total_frames - 1})"
                )
            ...
            if self.verbose:
                logger.info(
                    f"Extracting video range: frames {actual_frame_from}-{actual_frame_to} ({start_time_sec:.2f}s - {end_time_sec:.2f}s)"
                )
        ...
        feasibility = self._check_feasibility(memory_needed, video_meta.total_frames, is_video=True)
        ...
        try:
            ...
            if feasibility.requires_chunking:
                ...
                chunks = self._generate_chunks(video_meta, chunk_size, self.min_chunk_overlap)
                ...
                try:
                    ...
                    # Propagate
                    results, _, _ = driver.propagate_in_video(session_id, propagation_direction=propagation_direction)
            ...
            if self.verbose:
                ...
                if results:
                    ...
                    logger.info(
                        f"Sample frame {sample_frame_idx} structure: {type(sample_result)}, keys: {list(sample_result.keys()) if isinstance(sample_result, dict) else 'N/A'}"
                    )
    ...
    def process_video_with_points(
        ...
        # Validate input
        self._validate_video_input(video_path, points=points, point_labels=point_labels)
        ...
        try:
            ...
            try:
                ...
                results, _, _ = driver.propagate_in_video(
                    session_id, start_frame_idx=frame_idx, propagation_direction=propagation_direction
                )
            finally:
                driver.close_session(session_id)
    ...
    def remove_video_objects(
        self, video_path: str, object_ids: int | list[int], output_dir: str | None = None
    ) -> ProcessingResult:
    ...
    def process_video_with_segments(
        self, video_path: str, segments: dict[str, Any], output_dir: str | None = None
    ) -> ProcessingResult:
        ...
        try:
            ...
            for seg_idx, segment in enumerate(segments_list):
                ...
                try:
                    ...
                    results, _, _ = driver.propagate_in_video(
                        session_id, start_frame_idx=start_frame, propagation_direction="forward"
                    )

                    # Filter results to segment range
                    segment_results_filtered = {k: v for k, v in results.items() if start_frame <= k <= end_frame}
    ...
    def _process_video_chunk(
        ...
        try:
            ...
            chunk_results, _, _ = driver.propagate_in_video(
                session_id=session_id, propagation_direction=propagation_direction
            )
    ...
    def _process_video_in_chunks(
        ...
        # Extract first chunk
        if len(chunks) > 0:
            first_chunk_path = self._extract_video_chunk(video_path, chunks[0], str(chunk_dir / "chunk_0.mp4"))
        ...
        for i, chunk in enumerate(chunks):
            ...
            if i + 1 < len(chunks):
                ...
                extraction_thread = threading.Thread(target=extract_next_chunk, args=(chunks[i + 1],))
                # Don't start extraction immediately - will start at 90% progress

            if self.verbose:
                logger.info(f"Processing chunk {i + 1}/{len(chunks)} (frames {chunk.start_frame}-{chunk.end_frame})")
        ...
        all_results = self._merge_chunk_results(
            chunk_results=chunk_results_list, chunks=chunks, overlap=self.min_chunk_overlap
        )
    ...
    def _create_output_directory(self, base_dir: str, video_or_image_name: str) -> Path:
    ...
    def _save_image_masks(self, masks: np.ndarray, object_ids: list[int], output_dir: Path, prompt: str) -> list[str]:
    ...
    def _merge_chunk_results(
        self, chunk_results: list[dict[int, Any]], chunks: list[ChunkInfo], overlap: int
    ) -> dict[int, Any]:
        """
            ...
            if i == 0:
                ...
                for frame_idx, result in chunk_result.items():
                    ...
                    if frame_idx < chunk_info.start_frame + overlap:
                        if self.verbose:
                            logger.debug(f"Skipping overlapping frame {frame_idx} from chunk {i}")
                        continue
    ...
    def _get_image_driver(self) -> Sam3ImageDriver:
        ...
        if self._image_driver is None:
            self._image_driver = Sam3ImageDriver(bpe_path=self.bpe_path, num_workers=self.num_workers or 1)
        return self._image_driver

    def _get_video_driver(self) -> Sam3VideoDriver:
        """Lazy-load video driver."""
        if self._video_driver is None:
            self._video_driver = Sam3VideoDriver(bpe_path=self.bpe_path, num_workers=self.num_workers or 1)
...
def process_image_simple(
    image_path: str | list[str], prompts: str | list[str], output_dir: str | None = None
) -> ProcessingResult:
    """
...
def process_video_simple(video_path: str, prompts: str | list[str], output_dir: str | None = None) -> ProcessingResult:
    """
