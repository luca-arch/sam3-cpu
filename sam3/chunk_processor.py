...
class ChunkProcessor:
    ...
    def process_with_prompts(
        ...
        prev_chunk_data: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        """
    ...
    def _process_single_prompt(
        ...
        if prev_chunk_data and prompt in prev_chunk_data.get("masks", {}):
            ...
            if prev_masks:
                ...
                result_prompt, object_ids, id_mapping, global_next_id, iou_matrix = self._match_and_remap_ids(
                    result_prompt,
                    object_ids,
                    prev_masks,
                    global_next_id,
        ...
        if num_objects > 0:
            self._save_prompt_masks(
                result_prompt=result_prompt, object_ids=object_ids, prompt_masks_dir=prompt_masks_dir
            )
        ...
        prompt_metadata = {
            ...
            "iou_matrix": {str(k): {str(pk): v for pk, v in row.items()} for k, row in iou_matrix.items()}
            if iou_matrix
            else None,
    ...
    def _save_prompt_masks(self, result_prompt: dict, object_ids: set, prompt_masks_dir: Path):
        """
        ...
        for frame_idx in range(total_frames):
            ...
            for object_id in object_ids:
                ...
                if mask_uint8.shape != (height, width):
                    logger.error(
                        f"Invalid mask shape {mask_uint8.shape}, expected ({height}, {width}). Creating blank mask."
                    )
                    mask_uint8 = np.zeros((height, width), dtype=np.uint8)
    ...
    def _extract_carry_forward_data(self, prompt_results: dict[str, dict[str, Any]]) -> dict[str, Any]:
        """
        ...
        for prompt, result in prompt_results.items():
            ...
            if prompt_masks:
                ...
                logger.debug(f"carry_forward: extracted {len(prompt_masks)} mask(s) for prompt '{prompt}'")
